 ...
 */
class Absolute_Db_Table_NestedSet extends Zend_Db_Table {
    ...
    /**
     * __construct() - For concrete implementation of Absolute_Db_Table_NestedSet
     *
    ...
    protected function _setupPrimaryKey() {
        ...
        if (count($this->_primary) > 1) { //Compound key?
            include_once('Absolute/Db/Table/NestedSet/Exception.php');
            throw new Absolute_Db_Table_NestedSet_Exception('Tables with compound primary key are not currently supported.');
        }
    }
    ...
    protected function _setupLftRgt() {
        if (!$this->_left || !$this->_right) {
            include_once('Absolute/Db/Table/NestedSet/Exception.php');
            throw new Absolute_Db_Table_NestedSet_Exception('Both "left" and "right" column names must be supplied.');
        }
        ...
        if (count(array_intersect(array($this->_left, $this->_right), array_keys($this->_metadata))) < 2) {
            include_once('Absolute/Db/Table/NestedSet/Exception.php');
            throw new Absolute_Db_Table_NestedSet_Exception('Supplied "left" and "right" were not found.');
        }
    }
    ...
    public function getTree($where = null) {
        ...
        $select = $this->select()->setIntegrityCheck(false)
                ->from(
                        array(self::LEFT_TBL_ALIAS => $this->_name), array(self::LEFT_TBL_ALIAS . '.*', 'depth' => new Zend_Db_Expr('COUNT(' . self::RIGHT_TBL_ALIAS . '.' . $primary . ') - 1'))
                )
                ->join(
                        array(self::RIGHT_TBL_ALIAS => $this->_name), '(' . self::LEFT_TBL_ALIAS . '.' . $leftCol . ' BETWEEN ' . self::RIGHT_TBL_ALIAS . '.' . $leftCol . ' AND ' . self::RIGHT_TBL_ALIAS . '.' . $rightCol . ')', array()
                )
    ...
    public function insert(array $data, $objectiveNodeId = null, $position = self::LAST_CHILD) {
        if (!$this->_checkNodePosition($position)) {
            include_once('Absolute/Db/Table/NestedSet/Exception.php');
            throw new Absolute_Db_Table_NestedSet_Exception('Invalid node position is supplied.');
        }
    ...
    protected function _getCurrentObjectiveId($nodeId, $position) {
        ...
        $sql = $this->select()
                ->from(
                        array('node' => $this->_name), array($this->_primary[1])
                )
    ...
    /**
     * Fetch Row By ID. For compound primary key pass arguments in array in exact order
     * @author Qadeer Ahmad
     * @copyright Statigic System International
     * @param array $id integer or array
